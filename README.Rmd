...
knitr::include_graphics('C:/Users/Fernando-pc/Dropbox/spSUR/notes/Journal of Statisitical Software/Figure1_JSS.png')
```
...
R1 <- matrix(c(0,0,0,1,0,0,0,-1),nrow=1)
r1 <- matrix(0,ncol=1)
spcSUR.sar.restring <- spsurml(Form=Tformula, data=spc, type="sar", W=Wspc,R=R1,r=r1)
...
```{r}
# Tformula <- WAGE83 | WAGE81 ~ UN83 + NMR83 + SMSA | UN80 + NMR80 + SMSA
# spcSUR.sar <-spsurml(Form=Tformula,data=spc,type="sar",W=Wspc,trace=F)
...
r <- matrix(0,ncol=1)
LR_SMSA <-  lr_betas_spsur(Form=Tformula,data=spc,W=Wspc,type="sar",R=R,r=r,trace=F,printmodels=F)
```
...
```{r}
eff.spcSUR.sar <-impacts(spcSUR.sar,nsim=299)
```
...
A **SUR-SAR-PANEL** model  
$y_{gt} = \lambda_{g} Wy_{gt} + X_{gt} \beta_{g} + \epsilon_{gt};$   
$\ g=1,...,G; \ t=1,...,T$  
$Corr(\epsilon_{gt},\epsilon_{g't})=Corr(\epsilon_{g},\epsilon_{g'}) \neq 0$ for $(\forall t)$

```{r}
Tformula <- HR | HC ~ UE + RD | UE
spSUR.sar.panel <- spsurml(Form = Tformula,data=NATpanel,W=W,type="sar",nR=3085,nG=2,nT=4)
...
```{r}
Tformula <- HR | HC ~ UE + RD | UE
spSUR.sar.panel <- spsurml(Form = Tformula,data=NATpanel,W=W,type="sar",nR=3085,nG=2,nT=4,demean = T)
summary(spSUR.sar.panel)
```
