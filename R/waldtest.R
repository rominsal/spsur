...
wald_test <- function(betas,deltas,cov,p,nG){
    H1 <- NULL
    H2 <- NULL
    for (i in 2:p){
      ...
      e1[1,i] <- 1
      e2[1,i] <- -1
      block1 <- matrix(rep(e1,nG-1),ncol=p,byrow=TRUE)
      block2 <- matrix(c(e2,rep(e3,nG-2)),ncol=p,byrow=TRUE)
      H1 <- rbind(H1,block1)
      H2 <- rbind(H2,block2)
      H_betas <- cbind(H1,H2)
    }
