...
###############################################
  cov_spsurslm <- function(env){
    if(!is.null(env$W)) W <- env$W else W <- as(env$listw, "CsparseMatrix")
    G <- env$G; N <- env$N; Tm <- env$Tm
    Y <- env$Y; X <- env$X
    #YY <- matrix(Y, nrow = G*N) # LA USAMOS LUEGO PARA A*Y (CHEQUEAR PORQUÉ...)
    IT <- Matrix::Diagonal(Tm)
    IR <- Matrix::Diagonal(N)
    IG <- Matrix::Diagonal(G)
    IGR <- Matrix::Diagonal(G*N)
    ...
    k2 <- kronecker(Sigma,IR) %*% Auxit
    for (i in 1:G){
      # Cambiar  en función de tamaño W. El valor de order influye menos...
      IAgW <- spatialreg::powerWeights(W, deltas[i,i], X=W, 
                                                    order=250, tol = 0.001) 
      ...
      for (j in 1:G){
        if (i>j){I22A[i,j]<-I22A[j,i]}
          ...
          tXBslm_HH_XBslm <- Matrix::crossprod(XBslm,kk1)
          rm(kk1)
          ## PROBLEMA VIP: NO PUEDO UTILIZAR NI AUXIT NI SOLVE(AUXT,...)
          ## SE ME DISPARA EL TAMAÑO DEL VECTOR
          #k1 <- Auxit %*% kronecker(E[,,j,j]%*%Sigmainv%*%E[,,i,i],WtW)
          k1 <- Matrix::solve(Auxt,
                              kronecker(E[,,j,j]%*%Sigmainv%*%E[,,i,i],WtW))
          trace_HH_OME <- Tm*sum(k1*k2)
          if (i==j){
            I22A[i,j] <- Tm*sum(IAgW * Matrix::t(IAgW)) + tXBslm_HH_XBslm +
              trace_HH_OME
