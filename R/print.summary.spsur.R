...
#' @seealso
#' \itemize{
#'   \item \code{\link{summary.spsur}} Summary method for \emph{spsur} objects.
#' }
#'
#' @examples
...
print.summary.spsur <- function(x, digits = max(3L, getOption("digits") - 3L),
  ...
  if(!is.null(x$BP)){
    cat("\n Breusch-Pagan: ",formatC(x$BP, digits=5),
        "  p-value: (",formatC(pchisq(x$BP,df=nG*(nG-1)/2,lower.tail=FALSE),
                               digits=3, width=4),") ",sep="")

  }
  if(!is.null(x$LMM)){
    cat("\n LMM: ",formatC(x$LMM, digits=5),
        "  p-value: (",formatC(pchisq(x$LMM,df=nG*(nG-1)/2,lower.tail=FALSE),
                               digits=3, width=4),")\n",sep="")
