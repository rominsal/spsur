f_sur_lag <- function(deltag,nT,nG,nR,Y,X,W,Sigma)
{
  ...
  llike <- as.numeric(-(nT*nG*nR/2)*log(2*pi) - (nR*nT/2)*ldet_Sigma 
           + nT*sum(lDAg) - (1/2)*t(Res)%*%solve(OME,Res))
}   

LMSURquedaSEM <- function(nT,nG,nR,Y,X,W)
{
  ## PURPOSE: 
  ...
  #---------------------------------------------------
  # where: 
  #        nT   = numero de periodos temporales
  #        nG   = numero de ecuaciones que correlacionan (sectores)
  #        nR   = numero de elementos del corte transversal.
  ...
  B_sar <- Matrix::Matrix(solve(t(X)%*%solve(OME,X),
                   t(X)%*%solve(OME,as.vector(A%*%Y))))
  Res_sar <- matrix(A%*%Y - X%*%B_sar,nrow=nrow(Y))
  Yhat <- Y - Res_sar
  RR <- array(Res,dim=c(nR,nG,nT))
  for (i in 1:nG){
    for (j in 1:nG){
  ...
  res <- list(delta_sar = delta_sar,
              betas_sar = B_sar,
              llsur_sar = llsur_sarfin,
              Sigma_sar = Sigma,
              Sigma_corr_sar = Sigma_corr,
              Res_sar = Res_sar,
              Yhat_sar = Yhat
  )   
  # SLM MISLM
  ...
  for (i in 1:nG)
  {
    for (j in 1:nG)
    {
      ...
      if (i==j){
        ...
        I22A[i,j] <- as.numeric(nT*sum(diag(IAg%*%W%*%IAg%*%W)) + 
  ...
  I23A <- matrix(0,nrow=nG,ncol=nG*(nG+1)/2)
  #IA <- solve(A)
  for (i in 1:nG){
  ...
  I33A <- matrix(0,nrow=nG*(nG+1)/2,ncol=nG*(nG+1)/2)
  for (i in 1:(nG*(nG+1)/2)){
    for (j in 1:(nG*(nG+1)/2)){
      I33A[i,j]=(nT*nR/2)* sum(diag( (Sigma_inv%*%E[,,ff[i],cf[i]]) %*% 
  ...
  tstatTslm <- as.numeric(c(as.vector(Bslm), delta_sar)) / tmp[1:(fx+nG)]
  res$sd_beta_sar <- tmp[1:ncol(X)]
  res$sd_delta_sar <- tmp[(ncol(X)+1):(ncol(X)+nG)]
...
# grad rho
  ...
  Irhopslm <- cbind(matrix(0,nrow=nG,ncol=fx),
                    Ird, matrix(0,nrow=nG,ncol=nG*(nG+1)/2))
